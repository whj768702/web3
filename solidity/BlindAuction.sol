...
contract BlindAuction {
    ...
    /// 使用modifier可以更便捷的校验函数的入参
    /// onlyBefore 会被用于后面的 bid 函数
    /// 新的函数体是由 modifier本身的函数体， 并用原函数体替换 `_;` 语句来组成的
    ...
    /// 同一个地址可以放置多个出价。
    function bid(bytes32 _bindedBid) public payable onlyBefore(biddingEnd) {
        bids[msg.sender].push(
            Bid({blindedBid: _bindedBid, deposit: msg.value})
        );
    }
